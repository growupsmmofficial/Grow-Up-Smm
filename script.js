...
function initApp(){
    ...
    serviceList.addEventListener("click", handleCardActions);
}
...
function saveService(){
    ...
    if(!category){
        showToast("Category enter kar bhai");
        categoryInput.focus();
        return;
    }

    if(!name){
        showToast("Service name enter kar bhai");
        serviceInput.focus();
        return;
    }

    if(!link){
        showToast("Service link enter kar bhai");
        linkInput.focus();
        return;
    }

    if(!isValidUrl(link)){
        showToast("Valid link enter kar. Example: https://example.com");
        linkInput.focus();
        return;
    }
...
function getCategories(){
    const categories = services.map((item) => item.category || "General");
...
function renderCategorySelect(){
    ...
    if(getCategories().includes(selected)){
...
function openSelectedService(){
    ...
    if(!service){
        showToast("Pehle service select kar");
        return;
    }
...
function renderServiceList(){
    ...
    serviceList.innerHTML = filtered.map((item) => `
        <div class="service-card">
            ...
            <p class="service-link-text">${escapeHTML(item.link)}</p>
            <div class="card-actions">
                <button type="button" class="open-small" data-action="open" data-id="${escapeHTML(item.id)}">Open</button>
                <button type="button" class="edit-small" data-action="edit" data-id="${escapeHTML(item.id)}">Edit</button>
                <button type="button" class="delete-small" data-action="delete" data-id="${escapeHTML(item.id)}">Delete</button>
            </div>
        </div>
    `).join("");
}
...
function importData(event){
    ...
    reader.onload = function(){
        try{
            ...
            })).filter((item) => item.link && isValidUrl(item.link));
